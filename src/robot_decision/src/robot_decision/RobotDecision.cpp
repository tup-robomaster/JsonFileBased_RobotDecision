...
{
    int RobotDecision::calculatePosition(RobotPosition &pos)
    {
    ...
    bool RobotDecision::decodeWayPoints(char *filePath)
    {
        ...
        for (int i(0); i < int(arrayValue.size()); ++i)
        {
            ...
            wayPoint.name = arrayValue[i]["name"].asCString();
            ...
            {
                wayPoint.connect.emplace_back(connectedPoints[j].asInt());
            }
    ...
    bool RobotDecision::decodeDecisions(char *filePath)
    {
        Json::Reader jsonReader;
        Json::Value jsonValue;
        std::ifstream jsonFile(filePath);
    ...
    int RobotDecision::checkNowWayPoint(float x, float y)
    {
        RobotPosition pos;
        pos.x = x;
        pos.y = y;
    ...
    int RobotDecision::checkNowWayPoint(RobotPosition pos)
    {
        return this->calculatePosition(pos);
    }

    Decision RobotDecision::decide(int wayPointID, int robot_mode, int _HP, int nowtime, std::vector<RobotPosition> friendPositions, std::vector<RobotPosition> enemyPositions)
    {
